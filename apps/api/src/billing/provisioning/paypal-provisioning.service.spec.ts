...
describe('PayPalProvisioningService — first run', () => {
  ...
  /**
   * The two plans must be genuinely separate objects at PayPal. PayPal cannot
   * reprice a plan with live subscriptions, so "make a test price" can only ever
   * mean "make another plan" — never an edit to the $49 one.
   */
  it('creates the test plan at $1.00 alongside the published $49.00 plan', async () => {
    const { service, createPlan } = makeService();
    await service.provision();

    const amounts = createPlan.mock.calls.map(([input]) => (input as { amount: string }).amount);
    expect(amounts.sort()).toEqual(['1.00', '49.00']);

    // And the customer-visible name says which is which — PayPal shows it on the
    // approval page, so somebody paying $1 can tell it is not the normal price.
    const names = createPlan.mock.calls.map(([input]) => (input as { name: string }).name);
    expect(names).toEqual(
      expect.arrayContaining(['Founding Customer', 'Founding Customer (test pricing)']),
    );
  });
});
...
describe('PayPalProvisioningService — second run', () => {
  ...
  it('creates nothing when everything is already recorded and still exists', async () => {
    const { service, createProduct, createPlan, createWebhook } = makeService({
      ...
      plans: [
        { id: PLAN_ID, name: 'Founding Customer' },
  ...
  it('ignores an inactive plan of the same name rather than adopting it', async () => {
    const { service, createPlan } = makeService({
      plans: [
        { id: 'P-OLD', name: 'Founding Customer', status: 'INACTIVE' },
        { id: TEST_PLAN_ID, name: 'Founding Customer (test pricing)' },
      ],
    });
    await service.provision();
    ...
    expect(createPlan).toHaveBeenCalledWith(expect.objectContaining({ amount: '49.00' }));
  });
});
