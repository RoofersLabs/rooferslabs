...
function makeService(
  ...
  const config = {
    ...
    paypal: { environment: 'sandbox', clientId: 'AeA1QIZ-public-client-id' },
...
describe('BillingService — checkout', () => {
  ...
  it('creates a fresh customer when the stored one belongs to another provider', async () => {
    // A row left over from a retired processor names a customer that does not
    // exist at the current one; reusing the id would fail there. The enum no
    // longer carries such a member, so the historical value is asserted in —
    // exactly the shape a pre-migration row would have had.
    const { service, provider } = makeService({
      existing: makeRow({
        provider: 'STRIPE' as unknown as PaymentProvider,
...
describe('BillingService — lifecycle', () => {
  it('refuses to manage a subscription created by a different provider', async () => {
    // Its identifiers mean nothing to the active processor. Modelled with a
    // retired provider's value, which only a historical row could carry.
    const { service, provider } = makeService({
      existing: makeRow({ provider: 'STRIPE' as unknown as PaymentProvider }),
    });
...
describe('BillingService — public config', () => {
  it('carries no secret', () => {
    ...
    expect(JSON.stringify(config)).not.toMatch(/api[_-]?key|secret|token/i);
  });

  /**
   * Everything the browser is handed is either publishable or a redirect
   * destination, so nothing here can move money on its own. A field appearing
