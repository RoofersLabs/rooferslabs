...
export class BillingService {
  ...
  /**
   * What the browser needs to start a checkout. Contains no secrets.
   *
   * `clientId` is PayPal's *publishable* identifier — the same value that ships
   * in the public SDK URL of every PayPal integrator, analogous to Clerk's
   * `pk_…`. The browser needs it to load the JS SDK that renders the in-context
   * PayPal and card buttons; it cannot mint an access token or move money.
   * `PAYPAL_CLIENT_SECRET` is the credential, and it never leaves the server.
   *
   * `environment` is here purely so the UI can say out loud when it is pointed
