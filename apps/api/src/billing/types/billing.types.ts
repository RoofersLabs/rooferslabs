...
export interface CheckoutHandle {
  ...
  successUrl: string;
  /**
   * The provider's id for the pending subscription this checkout created.
   *
   * Exists for the in-context flow: PayPal's JS SDK approves a subscription by
   * id without leaving the page, and the confirm endpoint then reads that same
   * id back from PayPal. Carrying it grants nothing — activation still arrives
   * only through the verified webhook or the confirm read, both of which take
   * PayPal's word rather than the browser's.
   */
