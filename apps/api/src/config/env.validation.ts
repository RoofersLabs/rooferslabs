 ...
 *
 * A deployed process reading a `DATABASE_URL` of
 * `postgresql://…@localhost:5432/…` while `APP_ENV=development` says it is
 * talking to AWS is exactly the mistake this catches.
 */
...
export function validateEnv(config: Record<string, unknown>): Record<string, unknown> {
  ...
  const warnings = FEATURE_CREDENTIALS;
