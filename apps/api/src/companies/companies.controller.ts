...
export class CompaniesController {
  ...
  @Roles(UserRole.OWNER, UserRole.ADMIN)
  @ApiOperation({ summary: 'Set business hours' })
  ...
  @Roles(UserRole.OWNER, UserRole.ADMIN)
  @ApiOperation({ summary: 'Update the AI receptionist configuration' })
  ...
  @Roles(UserRole.OWNER, UserRole.ADMIN)
  @ApiOperation({ summary: 'Complete onboarding — the AI is ready to answer calls' })
  async complete(@CurrentCompanyId() companyId: string) {
    const company = await this.companies.completeOnboarding(companyId);
    return respond(company, 'Onboarding complete. Your AI receptionist is ready.');
  }
}
