...
RoofersLabs bills through **PayPal Subscriptions**. Stripe remains fully
implemented in the codebase but is dormant: never constructed, never loaded, and
refuses every call. Switching between them is a configuration change.
...
└── webhooks/
    ...
    └── stripe-webhook.controller.ts    Not registered while PayPal is active
```
...
1. **Never bound.** `BillingModule` registers exactly one adapter — the one
   `PAYMENT_PROVIDER` names — along with its collaborators. The dormant class is
   imported for its type but is not in the `providers` array, so Nest never
   constructs it.
...
| Column                   | Notes                                                       |
| ------------------------ | ----------------------------------------------------------- |
| `provider`               | `PAYPAL` \| `STRIPE` — makes every row self-describing      |
...
| `PAYPAL_TEST_PRICING`            | no       | Default `false`. `true` charges $1.00 instead of $49.00 — see below |
...
† Required in a **deployed** environment while `PAYMENTS_ENABLED=true`. Only the
_active_ provider's variables are enforced, which is what lets the platform boot
...
**Existing PayPal subscribers are not migrated.** Their rows stay
`provider = PAYPAL` and remain readable, but `BillingService` refuses to cancel
...
exercise, not something this switch attempts.
